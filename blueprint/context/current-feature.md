...
**Status:** Done — ready for `/complete` to archive it under `blueprint/history/features/`
**Goals:**
...
Rough priority order. Blueprint's home for these is `blueprint/build-plan.md` —
move them there as checkbox items when the build loop takes over, then run
`/feature` to spec the next one.
...
- [ ] **Improve Image Loading Time** - member photos load slowly; add placeholder until imported, and preload only visible images for first 9-12 members and await rest of photos in directory

---

## History

Reverse-chronological log of completed actions/milestones, carried over from the
pre-Blueprint `context/current-feature.md`. Going forward `/complete` archives
each finished feature as its own file under `blueprint/history/features/`.
